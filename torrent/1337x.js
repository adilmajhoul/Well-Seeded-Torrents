...
async function torrent1337x(query = "", page = "1") {
  ...
  await Promise.all(
    links.map(async (element) => {
      ...
      if (parseInt(data.Seeders) > 0) {
        allTorrent.push(data);
      }

      // sorting result by seeders
      //   if (allTorrentSorted.length === 0) {
      //     allTorrentSorted.push(data);
      //   } else {
      //     let i = 0;

      //     while (allTorrentSorted[i]) {
      //       if (data.Seeders >= allTorrentSorted[i].Seeders) {
      //         allTorrentSorted.splice(i, 0, data);
      //       }
      //       i++;
      //     }
      //   }
  ...
  console.log("🚀 allTorrentSorted:", allTorrentSorted);
